...
def chat_completions_common(body: dict, is_legacy: bool = False, stream=False) -> dict:
    ...
    if model in ['LLAMA-2-Chat-ITSM','LLAMA-2-Chat-Regular','LLAMA-3-Chat-ITSM','LLAMA-3-Chat-Regular']:
        ...
        raise InvalidRequestError(message="model picked is not supported", param='model')
    ...
    logger.debug(f'body of generate_params: \n {generate_params} ')
...
def chat_bot_common(body: dict, is_legacy: bool = False, stream=False) -> dict:
    ...
    if 'content' not in body:
        raise InvalidRequestError(message="content is required", param='content')
...
def chat_bot(body: dict, is_legacy: bool = False) -> dict:
    generator = chat_bot_common(body, is_legacy, stream=False)
    return deque(generator, maxlen=1).pop()
...
def stream_chat_bot(body: dict, is_legacy: bool = False):
    for resp in chat_bot_common(body, is_legacy, stream=True):
        yield resp
